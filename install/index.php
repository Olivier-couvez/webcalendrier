...
if( ! empty( $post_action ) && $post_action == $testSettingsStr && !
  ...
  if( $c ) {
    ...
    elseif( $db_type == 'mssql' ){
      $serverName = $db_host; //serverName\instanceName, portNumber (1433 by default)
      $connectionInfo = array( "Database"=>$db_database, "UID"=>$db_login, "PWD"=>$db_password);
      $c = sqlsrv_connect( $serverName, $connectionInfo);}
    elseif( $db_type == 'mysql' )
      $c = mysqli_connect( $db_host, $db_login, $db_password );
...
if( ! empty( $post_action2 ) && $post_action2 == $createNewStr && !
  ...
  elseif( $db_type == 'mssql' ) {
    ...
    if( $c ) {
      dbi_execute( $sql . ';', array(), false, $show_all_errors );
      if( ! mysqli_select_db($c, $db_database ) )
        $response_msg = $failure . dbi_error() . '</blockquote>' . "\n";
      else {
        $_SESSION['db_noexist'] = false;
        $_SESSION['old_program_version'] = 'new_install';
