    ...
    if ($res) {
        ...
        while ($row = dbi_fetch_row($res)) {
            ...
            <li>';
                    ...
                    $conflicts .= ($row[5] == 'C' && $row[0] != $login && !$is_assistant && !$is_nonuser_admin
    ...
    for ($q = 0; $q < $partcnt; $q++) {
        ...
        for ($i = 0; $i < $datecnt; $i++) {
            ...
            for ($j = 0, $listcnt = count($list); $j < $listcnt; $j++) {
                ...
                $row = $list[$j];
                if (
                    $row->getID() != $id && !in_array($row->getID(), $found)
      ...
      <tr>';
        ...
        <td';
            ...
            if (
                ...
                $ret_events = '';
                if (!$demo) {
                    $ret_events = print_date_entries(
                        $dateYmd,
                        (empty($user) ? $login : $user),
                        false,
                        true
                    );
                } else {
...
function display_navigation($name, $show_arrows = true, $show_cats = true)
{
    ...
    elseif ($name == 'month' || $name == 'view_l') {
        $ret .= $spacer
            . date_to_str(
                sprintf("%04d%02d01", $thisyear, $thismonth),
                $DATE_FORMAT_MY,
                false,
                false,
                true
 ...
 */
function display_small_month(
    $thismonth,
    $thisyear,
    $showyear,
    $show_weeknums = false,
    $minical_id = '',
    $month_link = 'month.php?'
) {
      ...
      <thead>
        <tr class="monthnav">
          <th colspan="' . ($DISPLAY_WEEKNUMBER == true ? 8 : 7) . '">
            ...
            . date_to_str(
                sprintf("%04d%02d%02d", $thisyear, $thismonth, 1),
...
function display_small_tasks($cat_id)
{
    ...
    $task_list = query_events(
        $task_user,
        false,
        (empty($task_filter) ? '' : $task_filter),
        $cat_id,
        true
    );
    $row_cnt = 1;
    ...
    foreach ($task_list as $E) {
        ...
        if (access_is_enabled()) {
            $can_access = access_user_calendar(
                'view',
                $task_owner,
                '',
                $E->getCalType(),
...
function display_unapproved_events($user)
{
    ...
    if (access_is_enabled()) {
        ...
        for ($j = 0, $cnt = count($all); $j < $cnt; $j++) {
            $x = $all[$j]['cal_login'];
            if (
                access_user_calendar('approve', $x) &&
...
function generate_refresh_meta()
{
    ...
    return ($AUTO_REFRESH == 'Y' && !empty($AUTO_REFRESH_TIME) && !empty($REQUEST_URI)
        ? '
    ...
    if ($rpt_type && (floor($date / 86400) * 86400) < $realend) {
        ...
        if ($rpt_type == 'daily') {
            ...
            while ($cdate <= $realend && $n < $Count) {
                // Check RRULE items.
                if (!empty($bymonth) && !in_array(date('n', $cdate), $bymonth))
                    $date_excluded = true;

                if (!empty($byweekno) && !in_array(date('W', $cdate), $byweekno))
                    $date_excluded = true;
            ...
            while ($cdate <= $realend && $n < $Count) {
                if (!empty($byday)) {
                    ...
                    for ($i = $WkstDay; $i <= ($WkstDay + 6); $i++) {
                        ...
                        if (
                            in_array($byday_names[$tdDay], $byday) && $td >= $date
                            && $td <= $realend && $n < $Count
                        )
                            $ret[$n++] = $td;
                    }
            ...
            while ($cdate <= $realend && $n < $Count) {
                ...
                if (isset($bymonthday))
                    $bymonthdayvalues = get_bymonthday(
                        $bymonthday,
                        $mdate,
                        $date,
                        $realend
          ...
          if (!empty($yret)) { // Add all BYxx additional dates.
            ...
            if (($n < $Count) && ($cdate >= $date))
                ...
                if (isset($bysetpos)) {
                    sort($yret);
                    for (
                        $i = 0, $bysetposcnt = count($bysetpos);
                        $i < $bysetposcnt;
                        $i++
                    ) {
                        $ret[] = ($bysetpos[$i] > 0
...
function get_byday($byday, $cdate, $type = 'month', $date)
{
    ...
    if (is_numeric($dayOffset)) { // Offset from end of $type.
            ...
            $byxxxDay = mktime(
                ...
                0,
                $month + 1,
                (0 - (($ldow + $dowOffset) % 7) + $dayOffsetDays),
                $yr
            );
            if ($mth == date('m', $byxxxDay) && $byxxxDay > $date)
                $ret[] = $byxxxDay;
...
function get_bymonthday($bymonthday, $cdate, $date, $realend)
{
    ...
    foreach ($bymonthday as $monthday) {
        $byxxxDay = mktime(
            ...
            0,
            $mth,
            ($monthday > 0 ? $monthday : $dim + $monthday + 1),
            $yr
        );
        if ($byxxxDay > $date)
            $ret[] = $byxxxDay;
...
function get_my_nonusers($user = '', $add_public = false, $reason = 'invite')
{
    ...
    if (
        !empty($my_nonuser_array[$this_user . $add_public]) &&
...
function get_my_users($user = '', $reason = 'invite')
{
    ...
    // Return the global variable (cached).
    if (
        !empty($my_user_array[$this_user][$reason]) &&
...
function getOverLap($item, $i, $parent = true)
{
    ...
    $midnight = gmmktime(
        - (date('Z', $item->getDateTimeTS()) / 3600),
        0,
        0,
        $lt[4] + 1,
        $lt[3] + 1,
...
function html_for_event_day_at_a_glance($event, $date)
{
  ...
  if ($time >= 0 && !$isAllDay && $getCalTypeName != 'task') {
    ...
    $hour_arr[$ind] .= build_entry_label(
        $event,
        'eventinfo-' . $linkid,
        $can_access,
        $popup_timestr,
 ...
 */
function html_for_event_week_at_a_glance(
    $event,
    $date,
    $override_class = '',
    $show_time = true
...
function load_user_layers($user = '', $force = 0)
{
    ...
    if ($force || (!empty($LAYERS_STATUS) && $LAYERS_STATUS != 'N')) {
        $rows = dbi_get_cached_rows(
            'SELECT cal_layerid, cal_layeruser, cal_color,
...
function print_entry($event, $date)
{
  ...
  if (!empty($categories[$catNum]['cat_color'])) {
    ...
    return $ret . build_entry_label(
        ...
        . (!empty($location) && !empty($DISPLAY_LOCATION) && $DISPLAY_LOCATION == 'Y'
  ...
  if (!empty($cat_id))
    ...
    if ($rows) {
        ...
        for ($ii = 0, $cnt = count($rows); $ii < $cnt; $ii++) {
            ...
            if (
                $login == '__public__' && !empty($OVERRIDE_PUBLIC) &&
                $OVERRIDE_PUBLIC == 'Y'
            ) {
  ...
  WHERE cal_id = ?', [$result[$i]->getID()]);
                ...
                if (!$result[$i]->getClone()) {
                    ...
                    if (count($result[$i - 1]->getRepeatAllDates()) > 0) {
                        ...
                        $cloneRepeats = [];
                        for (
                            $j = 0, $parentRepeatscnt = count($parentRepeats);
                            $j < $parentRepeatscnt;
                            $j++
                        ) {
                            $cloneRepeats[] = gmdate(
...
function sort_users($a, $b)
{
    ...
    $first = strnatcmp(
        strtolower($a['cal_firstname']),
        strtolower($b['cal_firstname'])
    );
    $last = strnatcmp(
        strtolower($a['cal_lastname']),
...
function user_is_participant($id, $user)
{
    ...
    $rows = dbi_get_cached_rows(
        'SELECT COUNT( cal_id ) FROM webcal_entry_user
...
function build_entry_popup(
    ...
    $details = true;
    if (
        function_exists('access_is_enabled') &&
...
function get_site_extras_names($filter = '')
{
    ...
    foreach ($site_extras as $extra) {
        if (
            $extra == 'FIELDSET' ||
            (!empty($extra[5]) && !empty($filter) && !($extra[5] & $filter))
        )
            continue;
...
function require_valid_referring_url()
{
    ...
    if (
        isset($settings['disable_referer_check']) &&
